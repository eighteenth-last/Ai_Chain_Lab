<template>
  <div class="notice-manage-container">
    ...
    <el-table :data="noticeList" v-loading="loading" stripe class="notice-table">
        ...
        <el-table-column label="操作" width="260" fixed="right">
          <template #default="scope">
            <div class="action-buttons">
              <el-button size="small" @click="viewNoticeDetail(scope.row)" icon="View" link>查看</el-button>
              <el-button size="small" type="primary" @click="editNotice(scope.row)" icon="Edit" link>编辑</el-button>
              <el-button size="small" :type="scope.row.isActive ? 'warning' : 'success'" @click="toggleStatus(scope.row)" icon="Switch" link>{{ scope.row.isActive ? '禁用' : '启用' }}</el-button>
              <el-button size="small" type="danger" @click="deleteNotice(scope.row)" icon="Delete" link>删除</el-button>
            </div>
          </template>
        </el-table-column>
      </el-table>
