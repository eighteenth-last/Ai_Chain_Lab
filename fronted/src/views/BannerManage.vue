<template>
  <div class="banner-manage-container">
    ...
    <el-table :data="bannerList" v-loading="loading" stripe class="banner-table">
        ...
        <el-table-column label="操作" width="240" fixed="right">
          <template #default="scope">
            <div class="action-buttons">
              <el-button size="small" type="primary" @click="editBanner(scope.row)" icon="Edit" link>编辑</el-button>
              <el-button size="small" :type="scope.row.isActive ? 'warning' : 'success'" @click="toggleBannerStatus(scope.row)" icon="Switch" link>{{ scope.row.isActive ? '禁用' : '启用' }}</el-button>
              <el-button size="small" type="danger" @click="deleteBanner(scope.row)" icon="Delete" link>删除</el-button>
            </div>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="600px" :close-on-click-modal="false">
      <el-form :model="bannerForm" :rules="bannerRules" ref="bannerFormRef" label-width="100px">
        ...
        <el-form-item label="图片" prop="imageUrl">
          ...
          <div v-if="bannerForm.imageUrl" class="preview-container">
...
import { Upload, Plus, Refresh, Edit, Switch, Delete, Picture, View, Link } from '@element-plus/icons-vue'
...
const handleUploadSuccess = (response, file) => {
  ...
  if (response.code === 200) {
    ...
    console.log('图片URL已设置:', bannerForm.imageUrl)
